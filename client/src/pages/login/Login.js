...
function Login({setIsAuthenticated}) {
  ...
  const handleSignIn = async (e) => {
    ...
    const response = await axios
     ...
     .post(`${link}/users/login`,userData)
      .catch((err) => {
        console.error(err);
      });
    ...
    try {
      if (response.status == 200) {
        ...
        // const userDetails = await axios.get("https://investnow-backend.herokuapp.com/users/fetchUserByEmail/?
         const userDetails = await axios.get(`${link}/users/fetchUserByEmail/?email=` +  localStorage.getItem("user"));       
